...
アプリケーションコードをバンドル・変換せず、静的ホスティングとブラウザーからの直接表示を維持します。`data.js` の生成方式は [データ圧縮の設計](browser-data-encoding.md) にまとめています。
...
[performance.spec.js](../tests/e2e/performance.spec.js) は、保存状態のない初回表示でリソースの `decodedBodySize` 合計を550,000バイト未満に制限します。通信時の圧縮後サイズや、全ページを閲覧した後の合計とは別の値です。
...
1. 変更範囲に応じた上記の検査を行う。
2. データを変更した場合は正本と生成物、READMEの掲載件数をそろえる。コンテンツ版・更新日は `data/manifest.json`、サイトマップの更新日は `sitemap.xml` で整合させる。
3. 読み込むデータ・スクリプト・CSSを変更した場合は、`index.html` の全ローカルスクリプトとCSSの `?v=` を同じ新しい値へ更新する。
4. `npm run build:site` で公開検査を通し、生成したdistを確認する。公開する内容は `dist/` のみ。
5. コミット・push後、対象コミットのCI、Deploy to Cloudflare Pages、Production smokeを確認する。
6. 本番で変更箇所を実操作し、公開物が対象コミットに一致することを確認する。

ドキュメントだけの変更で歴史データや読み込むアセットが変わらない場合、コンテンツ版・アセット識別子は維持できます。生成物を手で調整して検査を通さないでください。
...
`--wait` は配信反映を待って再試行します。照合対象はアプリ・データ・肖像の主要ファイルとHTTPヘッダーであり、公開物の全ファイルではありません。READMEや出典一覧などを変更した場合は、その配信内容も別途確認します。未コミットの変更があるチェックアウトとの比較は不一致になり得ます。
