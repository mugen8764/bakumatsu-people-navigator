...
- 活動分野: 1（医療・学問）
- 時点・主要事件: 16
- 個別事件: 5（日米修好通商条約の調印・八月十八日の政変・池田屋事件・大政奉還の上表・会津戦争の籠城）
- 人物関係: 80
- 勢力関係: 18
- 地点: 29
- 背景解説: 12項目
...
- 西郷・木戸・慶喜の転換点で、前後の名前・所属・役職・行動を比較
...
`main` のCI成功後は、CIが生成した `public-site` artifactをCloudflare Pagesへ配置します。配信成功後、本番の主要31ファイルが同じコミットの内容になったことと、4種のセキュリティヘッダー、16件のキャッシュ方針を自動確認します。
