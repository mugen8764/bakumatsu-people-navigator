...
- 活動分野: 1（医療・学問）
- 時点・主要事件: 16
- 個別事件: 16（和親・通商条約、生麦・薩英戦争・下関、京都の政変、薩長提携、政権交替、江戸開城、北越・会津戦争）
- 人物関係: 104
- 勢力関係: 18
- 地点: 30
- 背景解説: 23項目
...
北越・箱館の次期拡充は、[北越の交渉と箱館の終戦](docs/northern-expansion-plan.md)を参照してください。外交編の範囲・掲載上の留保・検証結果は、[開国交渉から対外衝突への拡充](docs/next-expansion-plan.md)にまとめています。それ以前の拡充と理解度評価の課題は、[3テーマの情報拡充](docs/content-expansion-2026-09.md)を参照してください。
...
- 西郷・木戸・慶喜・高杉・小松・勝・容保・榎本の転換点で、前後の名前・所属・役職・行動を比較
...
`main` のCI成功後は、CIが生成した `public-site` artifactをCloudflare Pagesへ配置します。配信成功後、本番の主要38ファイルが同じコミットの内容になったことと、4種のセキュリティヘッダー、23件のキャッシュ方針を自動確認します。
