...
- 活動分野: 2（医療・学問、暮らし・支援）
- 時点・主要事件: 16
- 個別事件: 20（和親・通商条約、生麦・薩英戦争・下関、京都・土佐の政局、寺田屋の救援、薩長提携、政権交替、江戸開城、北越・会津・箱館戦争）
- 人物関係: 108
- 勢力関係: 18
- 地点: 30
- 背景解説: 25項目
...
寺田屋編は[寺田屋襲撃と救援・保護](docs/teradaya-support-plan.md)にまとめています。土佐編の範囲と検証は、[土佐藩内の対立と勤王党の処分](docs/tosa-expansion-plan.md)を参照してください。北越・箱館は[北越の交渉と箱館の終戦](docs/northern-expansion-plan.md)、外交編は[開国交渉から対外衝突への拡充](docs/next-expansion-plan.md)にまとめています。それ以前の拡充と理解度評価の課題は、[3テーマの情報拡充](docs/content-expansion-2026-09.md)を参照してください。
...
- 人物の転換点で、前後の名前・所属・役職・行動を比較
- 本名、通称、変名、後年の名前、役職を含む横断検索
- 人物中心の時点別相関図
- 個別事件の役割別人物カードと、関係の向きを区別した相関図
- 各勢力の史料肖像と、画像ごとの出典・利用条件の表示
- 人物・事件のそばで開ける短い背景解説
- 役職名から開ける老中・勘定奉行・藩主・家老の解説
- 勢力ごとの目的・立場・構成人物・他勢力との関係
- 人物と事件の関連地を重ねた日本地図
- 地点カードから関連人物・事件への直接移動
- 原因・争点・結果をつないだ事件タイムライン
- URLハッシュによる時点・表示・人物・勢力・個別事件の共有
- 出典・編集方針のページ
...
`_headers` はNetlifyとCloudflare Pages向けです。セキュリティヘッダーに加え、HTML・静的データ・スクリプト・人物肖像は更新を再検証し、OG画像とアイコンは1日キャッシュする方針を定義しています。GitHub Pagesなど、このファイルを解釈しない配信先では同等のHTTPヘッダーをCDNまたはリバースプロキシ側で設定してください。

`index.html` が読み込むスクリプトとスタイルシートは、同一の `?v=` トークンを共有します。一部だけ更新すると、新しいデータと古い描画コードが組み合わさる可能性があるため、`npm run check:release` がトークンの食い違いを検出します。
...
`main` のCI成功後は、CIが生成した `public-site` artifactをCloudflare Pagesへ配置します。配信成功後、本番の主要39ファイルが同じコミットの内容になったことと、4種のセキュリティヘッダー、24件のキャッシュ方針を自動確認します。
