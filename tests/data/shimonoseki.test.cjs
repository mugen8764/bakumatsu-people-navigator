...
test('Alcock is on leave during Neale’s term and leaves before Parkes’s scene', () => {
  ...
  assert.equal(domain.statusAt(alcock, 2), null);
  ...
  assert.match(domain.statusAt(alcock, 7).role, /再来日/);
  assert.equal(domain.statusAt(domain.getPerson('neale'), 7), null);
  assert.equal(domain.statusAt(alcock, 8), null);
  assert.ok(!domain.activeRelations(8).some(r => r.a === 'alcock' || r.b === 'alcock'));
});
...
test('the returning students have evidence-backed comparisons before the attack', () => {
  for (const id of ['ito', 'inoue']) {
    ...
    const comparison = domain.turningPointAt(person, 7);
    ...
    assert.equal(domain.turningPointAt(person, 6), null);
