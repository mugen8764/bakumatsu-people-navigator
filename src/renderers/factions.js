  ...
  function createFactionsRenderer(context) {
    ...
    function render() {
      ...
      $('#factionCards').innerHTML = names.map(name => {
        ...
        return `<button type="button" class="faction-card ${name === state.selectedFaction ? 'selected' : ''}" data-faction-card="${name}" aria-pressed="${name === state.selectedFaction}"><div class="faction-header"><div class="faction-dot" style="background:${metadata.color}">${metadata.short}</div><div><div class="name">${name}</div><div class="faction-count">表示人物 ${memberCount}名</div></div></div><p>${states[name].position}</p><div class="faction-count">目的：${states[name].goal}</div>${shared.reviewBadge(states[name].evidence)}</button>`;
      }).join('');
    ...
    function renderDetail() {
      ...
      <div class="snapshot"><strong>${shared.dateLabel(shared.scene())}の位置 ${shared.reviewBadge(factionState.evidence)}</strong>${factionState.position}</div>
      ...
      <div class="section"><h3>他勢力との関係</h3><div class="relations">${relations.length ? relations.map(relation => {
        ...
        return `<div class="rel"><button type="button" data-other-faction="${other}">${other}</button> — ${relation.label} ${shared.reviewBadge(relation.evidence)}<br><span class="muted">${relation.text}</span></div>`;
