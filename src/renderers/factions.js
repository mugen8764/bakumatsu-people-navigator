  ...
  function createFactionsRenderer(context) {
    ...
    function render() {
      ...
      $('#factionCards').innerHTML = names.map(name => {
        ...
        return `<button type="button" class="faction-card ${name === state.selectedFaction ? 'selected' : ''}" data-faction-card="${esc(name)}" aria-pressed="${name === state.selectedFaction}"><div class="faction-header"><div class="faction-dot" style="background:${esc(metadata.color)}">${esc(metadata.short)}</div><div><div class="name">${esc(name)}</div><div class="faction-count">表示人物 ${memberCount}名</div></div></div><p>${esc(states[name].position)}</p><div class="faction-count">目的：${esc(states[name].goal)}</div>${shared.reviewBadge(states[name].evidence)}</button>`;
      }).join('');
    ...
    function renderDetail() {
      ...
      <div class="section"><h3>この時点の目的</h3><p>${esc(factionState.goal)}</p></div>
      <div class="section"><h3>勢力の基本像</h3><p>${esc(metadata.summary)}</p></div>
