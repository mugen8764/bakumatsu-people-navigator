(function exposeEventsRenderer(root, factory) {
  ...
  function createEventsRenderer(context) {
    ...
    const involvementLabels = { onsite: '現場で突入', decision: '別隊の指揮', context: '背景・制度' };
    ...
    function incidentDetail(incident) {
      ...
      return `<button type="button" class="button" data-open-overview>← ${esc(shared.scene().year)}年の概要へ</button>
        ...
        <section class="incident-roles" aria-labelledby="incidentRolesTitle"><div class="incident-section-heading"><h2 id="incidentRolesTitle">誰が、何をした？</h2><span>新選組側と京都守護職</span></div><p class="muted">現場への突入・別隊の指揮・背景を分けて表示しています。名前を選ぶと人物詳細へ進めます。</p>${groups}</section>
        <section class="incident-connections" aria-labelledby="incidentRelationsTitle"><h2 id="incidentRelationsTitle">この事件を読むための相関図</h2><p class="muted">線は記載した関係を表します。同じ事件に登場するだけでは結んでいません。</p><ul class="incident-relations">${relations}</ul></section>
        ...
        <p class="portrait-note">肖像は選択時点の姿とは限りません。国立国会図書館「近代日本人の肖像」より。</p>${incident.participants.map(item => shared.portraitCredit(domain.getPerson(item.personId))).join('')}`;
    }
